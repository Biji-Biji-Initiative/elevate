...
vi.mock('@elevate/auth/server-helpers', async () => {
  return {
    ...
    createErrorResponse: (err: unknown, status = 500) =>
      new Response(
        JSON.stringify({ error: (err as Error)?.message || 'err' }),
        { status },
      ),
  }
})
...
vi.mock('@elevate/db', async () => {
  return {
    // Service-layer functions used by the route
    findSubmissionsWithFilters: (...args: unknown[]) =>
      findManyMock(...(args as never)),
    countSubmissionsWithFilters: (...args: unknown[]) =>
      countMock(...(args as never)),
  }
})
...
describe('Admin submissions API - GET', () => {
  ...
  it('returns submissions with attachmentCount derived from relation', async () => {
    ...
    findManyMock.mockResolvedValueOnce([
      {
        ...
        attachments: [],
        attachments_rel: [
          { id: 'a1', submission_id: 'sub_1', path: 'evidence/p.pdf' },
        ],
    ...
    const req = new Request(
      'http://localhost/api/admin/submissions?status=ALL&page=1&limit=50',
    )
