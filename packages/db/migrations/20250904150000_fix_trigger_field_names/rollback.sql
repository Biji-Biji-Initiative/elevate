-- Rollback for trigger field names fix
-- Reverts to camelCase field names (old behavior)
...
BEGIN
  ...
  -- Extract new values from JSON payload (reverted to camelCase)
  new_peers := COALESCE( (NEW.payload->>'peersTrained')::int, 0 );
  new_students := COALESCE( (NEW.payload->>'studentsTrained')::int, 0 );

  -- Sum existing values in last 7 days (reverted to camelCase)
  SELECT
