...
export function CSPScript({
  ...
  defer,
  async,
}: CSPScriptProps): JSX.Element | null {
...
export function CSPStyle({
  ...
  id,
  media,
}: CSPStyleProps): JSX.Element | null {
  const nonce = useCSPNonce();
 ...
 */
export function NonceProvider({ nonce, children }: NonceProviderProps): JSX.Element {
  useEffect(() => {
 ...
 */
export function CSPMetaTags({ nonce }: CSPMetaTagsProps): JSX.Element {
  return (
    <>
...
export function withCSPNonce<P extends object>(
  ...
  const WrappedComponent = (props: P) => {
    ...
    return (
      <Component
        {...props}
        // @ts-expect-error - We're adding the nonce prop
        cspNonce={nonce}
      />
    );
  };

  WrappedComponent.displayName = `withCSPNonce(${Component.displayName || Component.name})`;
 ...
 */
export function executeWithNonce(code: string, options?: {
  type?: string;
  defer?: boolean;
  async?: boolean;
}): Promise<void> {
 ...
 */
export function hasNonce(element: HTMLElement): element is HTMLElement & { nonce: string } {
 ...
 */
export function setInnerHTMLWithNonce(element: HTMLElement, html: string): void {
  const nonce = document.querySelector('meta[name="csp-nonce"]')?.getAttribute('content');
