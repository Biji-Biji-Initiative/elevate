...
// Import function that needs to be available - assuming this exists in types package
...
export function ProfileCard({ user, rank, showRank = false, compact = false, getProfilePath }: ProfileCardProps) {
  const totalPoints = user._sum?.points || 0
  const profilePath = getProfilePath ? getProfilePath(user.handle) : `/u/${user.handle}`
