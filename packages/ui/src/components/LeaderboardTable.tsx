...
import { isString } from '@elevate/types'

// Use LeaderboardEntry from @elevate/types
// type LeaderboardEntry is imported from @elevate/types
    ...
    <div className="space-y-6">
      ...
      {filteredData.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">No educators found</h3>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedData.map((entry) => {
                  const profilePath = getProfilePath ? getProfilePath(entry.user.handle) : `/u/${entry.user.handle}`
                  ...
                  return (
                    <tr key={entry.user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3">
                            ...
                            <div className="text-xs text-gray-500">@{entry.user.handle}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-lg font-bold text-blue-600">
                        {entry.user._sum?.points?.toLocaleString() || 0}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-1">
                          {entry.user.earned_badges?.slice(0, 3).map((badge, idx) => (
                            <div 
                              key={idx}
                          ...
                          {(entry.user.earned_badges?.length || 0) > 3 && (
                            <div className="w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center">
                              <span className="text-xs text-gray-600">+{(entry.user.earned_badges?.length || 0) - 3}</span>
                            </div>
                          )}
                        </div>
                      </td>
