...
export class KajabiClient {
  ...
  async createOrUpdateContact(
    ...
    try {
      ...
      if (existingContact) {
        ...
        const response: AxiosResponse<KajabiContactResponse> =
          await this.client.put(`/contacts/${existingContact.id}`, {
            contact: contactData,
          })
        ...
        // Create new contact
        const response: AxiosResponse<KajabiContactResponse> =
          await this.client.post('/contacts', { contact: contactData })
        return response.data.contact
      }
    } catch (error: unknown) {
  ...
  async findContactByEmail(email: string): Promise<KajabiContact | null> {
    try {
      const response: AxiosResponse<KajabiContactsResponse> =
   ...
   * Accepts either a numeric ID or a string identifier, as Kajabi UIs may
   * expose non-numeric IDs.
   */
  async grantOffer(
    contactId: number,
    ...
    try {
      const response: AxiosResponse<KajabiOfferResponse> =
        await this.client.post(`/contacts/${contactId}/offers/${offerId}/grant`)
  ...
  async getContactTags(contactId: number): Promise<KajabiTag[]> {
    try {
      const response: AxiosResponse<KajabiTagsResponse> = await this.client.get(
...
export function getKajabiClient(): KajabiClient {
  if (!kajabiClient) {
    ...
    if (!apiKey || !clientSecret) {
      throw new Error(
        'KAJABI_API_KEY and KAJABI_CLIENT_SECRET must be set in environment variables',
      )
    }
...
export function createKajabiClient(
  apiKey: string,
  clientSecret: string,
): KajabiClient {
